...
def fixFiles(files: list[str]) -> list[str]:
    renamed_files = []
    duplicated = []
    for file in files:
        duplicated.append(file)

        if file in renamed_files: file = file + f"({duplicated.count(file) - 1})"
...
def main():
    ...
    print(test(['photo', 'postcard', 'photo(1)', 'photo(2)', 'video'], fixFiles(files))) # ['photo', 'postcard', 'photo(1)', 'photo(2)', 'video']

    files2 = ['file', 'file', 'file', 'game', 'game']
    print(test(['file', 'file(1)', 'file(2)', 'game', 'game(1)'], fixFiles(files2))) # ['file', 'file(1)', 'file(2)', 'game', 'game(1)']

    # ojo que los elfos ya tenían archivos con (1)... ¡y pueden estar repetidos!
    files3 = ['file', 'file(1)', 'icon', 'icon(1)', 'icon(1)']
    print(test(['file', 'file(1)', 'icon', 'icon(1)', 'icon(1)(1)'], fixFiles(files3))) # ['file', 'file(1)', 'icon', 'icon(1)', 'icon(1)(1)']
