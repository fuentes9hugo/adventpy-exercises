...
def fitsInOneBox(boxes: list[dict]) -> bool:
    ...
    return all(
        ...
        curr["w"] < nex["w"] and 
        curr["h"] < nex["h"]
        for curr, nex in zip(boxes, boxes[1:])
    )
...
def main():
    ...
    print(test(False, fitsInOneBox(boxes))) # false
    ...
    print(test(True, fitsInOneBox(boxes))) # true
