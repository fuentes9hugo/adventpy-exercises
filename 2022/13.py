...
def getFilesToBackup(lastBackup: int, changes: list[list[int]]) -> list[int]:
    return list({change[0] for change in changes if change[1] > lastBackup})
...
def main():
    ...
    print(test([1, 3], getFilesToBackup(lastBackup, changes))) # => [ 1, 3 ]
