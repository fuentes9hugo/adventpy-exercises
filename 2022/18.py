...
Nos debe devolver un array con los números que incluyen el número que no tenemos tinta.
...
def dryNumber(dry: int, numbers: int) -> list[int]:
    dry_str = str(dry)
    return [i for i in range(1, numbers + 1) if dry_str in str(i)]
...
def main():
    print(test([1, 10, 11, 12, 13, 14, 15], dryNumber(1, 15))) # [1, 10, 11, 12, 13, 14, 15]
    ...
    print(test([2, 12, 20], dryNumber(2, 20))) # [2, 12, 20]
