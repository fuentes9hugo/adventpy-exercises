...
def organizeGifts(gifts: str) -> str:
    matches = re.findall(r"(\d+)([a-z])", gifts)

    organized_gifts = []

    for num, char in matches:
        boxes, units = divmod(int(num), 10)
        pallets, boxes = divmod(boxes, 5)

        organized_gifts.append(f"[{char}]" * pallets + f"{{{char}}}" * boxes + "(" + f"{char}" * units + ")")

    return "".join(["".join(gift) for gift in organized_gifts])
...
def main():
    result1 = organizeGifts("76a11b")
    print(test("[a]{a}{a}(aaaaaa){b}(b)", result1))
    print(organizeGifts("76a11b"))
    # '[a]{a}{a}(aaaaaa){b}(b)'
